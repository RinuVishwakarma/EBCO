...
import DesignerCard from '../../module-components/DesignerInspiration/DesignerCard'
import Loader from '../Loader'
interface DesignerTabItem {
  id: number
  name: string
}
const Designer = ({ name, id }: DesignerTabItem) => {
  ...
  //API FOR Products(Need Change)
  const fetchVideosTabData = async (
    searchString: string,
  ): Promise<DownloadBrochure[] | []> => {
    try {
      const response = await apiClient.get<DownloadBrochure[]>(
        `${API_ENDPOINT.GET.getDesignersInspiration}?per_page=12&page=${page}&designer-inspiration-category=${id}&search=${searchString}&acf_format=standard${queryString}`,
      )
  ...
  const { isLoading, data } = fetchVideosQuery
  ...
  return (
    <Box className='brochure-container'>
      ...
      <Box className='brochure-wrapper'>
        ...
        <Box className='cards-container-designer'>
          {isLoading && <Loader />}
          ...
          {!fetchVideosQuery.isFetching &&
            fetchVideosQuery?.data?.map((item, index) => {
              //@ts-ignore
              return <DesignerCard item={item} key={index} />
            })}
        </Box>
      </Box>
      {fetchVideosQuery.data && (
        <Box
