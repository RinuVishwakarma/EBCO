...
import { useLightTextHeading, useLightTextSubHeading, useBoldTextSubHeading, useBoldTextHeading, useEbcoBorderBlueButtonStyle, useEbcoOrangeButtonStyle } from '@/utils/CommonStyling';
...
export default function MyProductRangeCarousal() {
  ...
  const homePageQuery = useQuery({
    queryKey: ["home-page-product-range-carousal"],
        ...
        homePageQuery.isLoading ? (
          ...
          <>
            <Swiper
              ...
              className="product-range-swiperr"
            >
              {productRanges?.map((product, index) => (
                <SwiperSlide key={index}>
                  ...
                  <Box
                    lazy="true"
                    className="product-range-card bg-center"
                    ref={boxRef}
                    sx={{
                      height: isMediumScreen ? '325px' : '450px',
                      width: '83vw !important',
                      cursor: 'grab',
                      margin: isMediumScreen ? '0 1rem' : '0 2rem',
                      position: 'relative',
                  ...
                  >
                    ...
                    >
                      ...
                      <Box className="logo-product-range mobile-section row-space-between"
                        sx={{
                          height: '50px',
                          width: '100%',
                      ...
                      >
                        <Image src={product.logo} alt='Ebco logo' width={50} height={25}
                          style={{
                      ...
                      <Image alt='Image' className="product_range_card_image" src={product.image} loading='lazy' width={1200} height={700}
                        style={{
                          position: 'absolute',
                          width: '100% !important',
                          height: '100% !important',
                      ...
                      {isMediumScreen
                        ...
                        >
                          ...
                          >
                            ...
                            <Box className="row-center" sx={{ width: '100%' }}>
                            ...
                            >
                              <Image src={product.logo} alt='Ebco logo' className='hoveredLogo-image' width={80} height={38} style={{ height: 'auto', width: '80px' }} onClick={() => routeToProductRange(product.route)}
                              />
                              <CloseIcon sx={{
                                width: '20px',
                                height: '20px',
                                ...
                                onClick={() => closeDescription(index)}
                              />
                            </Box>
                          </Box>
                          <Box className="logo-product-range row-space-between"
                            sx={{
                              height: '60px',
                              width: '90%',
                              cursor: 'pointer !important',
                        ...
                        >
                          <Box className="product_details w-100 column-space-between" sx={{ flex: '1' }}>
                           {product.logo && <Box className="hovered_logo_container ">
                              <Image onClick={() => routeToProductRange(product.route)} src={product.logo} className="unhovered_logo" alt={product.route + ' logo'} width={150} height={150} />
                            </Box>
                            }
                            <Typography sx={{
                              padding: '1rem',
                            ...
                            <Box className="detail-buttons w-100 column-space-between" sx={{ padding: '1rem' }}>
                              <button onClick={() => routeToProductRange(product.route)} style={{ ...ebcoOrangeButtonStyle, width: '90%', margin: '0.5rem auto', fontSize: '14px', height: '40px' }}>KNOW MORE</button>
                             {product.file && <button style={{ ...ebcoBorderBlueButtonStyle, width: '90%', margin: '0.5rem auto', fontSize: '14px', height: '40px', border: '1px solid lightgray' }}
                                onClick={() => {
                                  downloadBrochure(product.file);
                                }}
                              >Download Brochure</button>}
            ...
            <Box className="row-space-between w-100 custom-products-arrows">
              <button className="arrow-left-product arrow" aria-label='Previous'>
                {/* <Image src='/images/prev_icon.svg' alt="Previous icon" width={24} height={18} /> */}
                <WestIcon className='arrow' color='#5b5b5b' />
              </button>
              <button className="arrow-right-product arrow" aria-label='Next'>
