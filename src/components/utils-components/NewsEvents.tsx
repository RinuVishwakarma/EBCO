...
import NewsCard from "./EventsCard";
import EventsCarousal from "./MyEventsCarousal";
...
const NewsEventsSection = ({
  ...
  const [selectedVideo, setSelectedVideo] = useState<NewsEventData>();
  ...
  const fetchVideosTabData = async (
    ...
    try {
      const response = await apiClient.get<NewsEventData[]>(
        `${API_ENDPOINT.GET.getNewsEvents}?per_page=12&page=${page}&news-event-category=${id}&search=${searchString}${queryString}&status=publish`
      );
