...
const NewsAndEventsId = () => {
  ...
  const fetchVideosTabData = async (): Promise<DiscoveryCenter | undefined> => {
    try {
      const response = await apiClient.get<DiscoveryCenter>(
        `${API_ENDPOINT.GET.getDiscoveryCenter}/${params.id}?acf_format=standard`
      );
      ...
      if (Array.isArray(response.data.acf.carousel)) {
        // console.log("YES I AM ARRAY");
        images = response.data.acf.carousel;
      }
      images.unshift(response.data.featured_media_src_url);
      setImages(images);
  ...
  return (
    <>
      <head>
        <title>
          {selectedItem && decodeHtml(selectedItem?.title?.rendered!)}
        </title>
      </head>
      <Box className="news-events-container" p={2}>
        {!fetchVideosQuery.isFetching && (
          ...
          //     <img
          //       src={selectedItem?.featured_media_src_url}
